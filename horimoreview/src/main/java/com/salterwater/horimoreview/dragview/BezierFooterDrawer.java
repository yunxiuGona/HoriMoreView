...
public class BezierFooterDrawer extends BaseFooterDrawer {
    ...
    private String[] normalTextRows;
    private String[] eventTextRows;
    ...
    private void initTextRows() {
        ...
        int eventLength = drawParams.eventString.length();
...
//        int rowsLength = normalLength > eventLength ? normalLength : eventLength;
        normalTextRows = new String[normalLength];
        eventTextRows = new String[eventLength];
    }
    ...
    private void drawText(Canvas canvas) {
        ...
        setTextRows(canvas, x, y);
    }

    private void setTextRows(Canvas canvas, float x, float y) {
        if (drawParams.eventString == null || drawParams.eventString.isEmpty()) {
            drawParams.eventString = drawParams.normalString;
        }

        if (getDraggedDistance() > rotateThreshold) {
            String tmp = drawParams.eventString;
            for (int i = 0; i < tmp.length(); i++) {
                eventTextRows[i] = String.valueOf(tmp.charAt(i));
            }
            drawTextInRows(eventTextRows, canvas, x, y);
        } else {
            String tmp = drawParams.normalString;
            for (int i = 0; i < tmp.length(); i++) {
                normalTextRows[i] = String.valueOf(tmp.charAt(i));
            }
            drawTextInRows(normalTextRows, canvas, x, y);
        }
//        String tmp;
